// Copyright (c) 2023 The MobileCoin Foundation

#![doc = include_str!("../README.md")]

use core::{mem::size_of, result::Result as StdResult};
use displaydoc::Display;
use hex_fmt::HexFmt;
use nom::AsBytes;
use sha2::{Digest, Sha256};
use std::fmt::{self, Debug, Formatter};

type Result = StdResult<Signature, Error>;

const ZEROV: [u8; LENGTH_84] = [0; LENGTH_84];
const HEADER1: [u8; LENGTH_16] = [
    0x06, 0x00, 0x00, 0x00, 0xE1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
];
const HEADER2: [u8; LENGTH_16] = [
    0x01, 0x01, 0x00, 0x00, 0x60, 0x00, 0x00, 0x00, 0x60, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00,
];

/// The vendor enum used in the enclave
#[derive(Copy, Clone, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
#[repr(u32)]
pub enum EnclaveVendor {
    Intel = 0x0000_8086,
    Other = 0x0,
}

impl TryFrom<u32> for EnclaveVendor {
    type Error = Error;

    fn try_from(src: u32) -> StdResult<EnclaveVendor, Self::Error> {
        match src {
            0x0000_8086 => Ok(EnclaveVendor::Intel),
            0x0 => Ok(EnclaveVendor::Other),
            _unknown => Err(Error::UnknownVendor),
        }
    }
}

/// An enumeration of errors which can occur while parsing a Signature struct
#[derive(Copy, Clone, Debug, Display, Eq, Hash, Ord, PartialEq, PartialOrd)]
pub enum Error {
    /// Structure data has the wrong length
    Length,
    /// Invalid header value encountered
    BadHeader1,
    /// Invalid header value encountered
    BadHeader2,
    /// Unknown vendor encountered
    UnknownVendor,
    /// Enclave build date too old to be reasonable
    BadDate,
    /// Reserved bytes are not zero
    NonZeroReserved,
}

/// The length of the HEADER field, in bytes
pub const HEADER_LEN: usize = LENGTH_16;
/// The length of the VENDOR field, in bytes
pub const VENDOR_LEN: usize = LENGTH_4;
/// The length of the DATE field, in bytes
pub const DATE_LEN: usize = LENGTH_4;
/// The length of the HEADER2 field, in bytes
pub const HEADER2_LEN: usize = LENGTH_16;
/// The length of the SWDEFINED field, in bytes
pub const SWDEFINED_LEN: usize = LENGTH_4;
/// The length of the MODULUS field, in bytes
pub const PUBKEY_LEN: usize = LENGTH_384;
/// The length of the EXPONENT field, in bytes
pub const EXPONENT_LEN: usize = LENGTH_4;
/// The length of the SIGNATURE field, in bytes
pub const SIGNATURE_LEN: usize = LENGTH_384;
/// The length of the MISCSELECT field, in bytes
pub const MISCSELECT_LEN: usize = LENGTH_4;
/// The length of the MISCMASK field, in bytes
pub const MISCMASK_LEN: usize = LENGTH_4;
/// The length of the ATTRIBUTES field, in bytes
pub const ATTRIBUTES_LEN: usize = LENGTH_16;
/// The length of the ATTRIBUTEMASK field, in bytes
pub const ATTRIBUTEMASK_LEN: usize = LENGTH_16;
/// The length of the ENCLAVEHASH field, in bytes
pub const MRENCLAVE_LEN: usize = LENGTH_32;
/// The length of the hash of the signer values, in bytes
pub const MRSIGNER_LEN: usize = LENGTH_32;
/// The length of the ISVPRODID field, in bytes
pub const ISVPRODID_LEN: usize = LENGTH_2;
/// The length of the ISVSVN field, in bytes
pub const ISVSVN_LEN: usize = LENGTH_2;
/// The length of the Q1 signature verification value
pub const Q1_LEN: usize = LENGTH_384;
/// The length of the Q2 signature verification value
pub const Q2_LEN: usize = LENGTH_384;

const LENGTH_2: usize = 2;
const LENGTH_4: usize = 4;
const LENGTH_16: usize = 16;
const LENGTH_32: usize = 32;
const LENGTH_84: usize = 84;
const LENGTH_384: usize = 384;
const RESERVED1_LEN: usize = 84;
const RESERVED2_LEN: usize = 20;
const RESERVED3_LEN: usize = 32;
const RESERVED4_LEN: usize = 12;

/// The SIGSTRUCT structure
#[derive(Clone, Eq, Hash, Ord, PartialEq, PartialOrd)]
pub struct Signature {
    /// The byte stream 0x06000000E10000000000010000000000
    header: [u8; HEADER_LEN],
    /// Intel enclaves will use 0x00008086, other enclaves will use 0x00000000
    vendor: EnclaveVendor,
    /// The build date of the enclave, as an 8-decimal date (YYYYMMDD) stored as
    /// little-endian bytes
    date: [u8; DATE_LEN],
    /// The byte stream 0x01010000600000006000000001000000
    header2: [u8; HEADER2_LEN],
    /// Data defined by the signature software
    swdefined: [u8; SWDEFINED_LEN],
    /// The RSA 3072-bit public key
    modulus: [u8; PUBKEY_LEN],
    /// The RSA Exponent, as an LE u32
    exponent: [u8; EXPONENT_LEN],
    /// The RSA signature over the header and body
    signature: [u8; SIGNATURE_LEN],
    /// The Extended SSA frame feature set selection, as 32 bits
    miscselect: [u8; MISCSELECT_LEN],
    /// A bitmask of miscselect features, as 32 bits
    miscmask: [u8; MISCMASK_LEN],
    /// Enclave attribute flags
    attributes: [u8; ATTRIBUTES_LEN],
    /// Bitmask of attribute flags
    attributemask: [u8; ATTRIBUTEMASK_LEN],
    /// The MRENCLAVE value, as bytes
    enclavehash: [u8; MRENCLAVE_LEN],
    /// The product ID, stored as little-endian bytes
    isvprodid: [u8; ISVPRODID_LEN],
    /// The security version of the enclave, stored as little-endian bytes
    isvsvn: [u8; ISVSVN_LEN],
    /// Q1 value
    q1: [u8; Q1_LEN],
    /// Q2 value
    q2: [u8; Q2_LEN],
}

impl Signature {
    /// Retrieve a reference to the header bytes
    pub fn header(&self) -> &[u8; HEADER_LEN] {
        &self.header
    }

    /// Retrieve the enclave vendor type.
    pub fn vendor(&self) -> EnclaveVendor {
        self.vendor
    }

    /// Retrieve the date (without timezone) the enclave was built as a
    /// [Binary-Coded Decimal](https://en.wikipedia.org/wiki/Binary-coded_decimal).
    ///
    /// This means that a date like April 15, 2020 will be encoded as the
    /// hexadecimal 0x20200415. It's a terrible way of encoding dates, but
    /// it's how SIGSTRUCT works internally.
    pub fn date(&self) -> u32 {
        u32::from_le_bytes(self.date)
    }

    /// Retrieve a reference to the secondary header bytes
    pub fn header2(&self) -> &[u8; HEADER2_LEN] {
        &self.header2
    }

    /// Retrieve the software-defined bytes
    pub fn swdefined(&self) -> &[u8; SWDEFINED_LEN] {
        &self.swdefined
    }

    /// Retrieve the signer's RSA public key, as bytes
    pub fn pubkey_bytes(&self) -> &[u8; PUBKEY_LEN] {
        &self.modulus
    }

    /// Retrieve the RSA exponent
    pub fn pubkey_exponent(&self) -> u32 {
        u32::from_le_bytes(self.exponent)
    }

    /// Retrieve a reference to the signature bytes
    pub fn signature(&self) -> &[u8; SIGNATURE_LEN] {
        &self.signature
    }

    /// Retrieve the miscellaneous selection flags
    pub fn misc_select(&self) -> u32 {
        u32::from_le_bytes(self.miscselect)
    }

    /// Retrieve which miscellaneous selection flags are relevant
    pub fn misc_mask(&self) -> u32 {
        u32::from_le_bytes(self.miscmask)
    }

    /// Retrieve the attributes flags (64-bit flags + 64-bit xfrm)
    pub fn attributes(&self) -> u128 {
        u128::from_le_bytes(self.attributes)
    }

    /// Retrieve a mask indicating which attribute flags are relevant
    pub fn attribute_mask(&self) -> u128 {
        u128::from_le_bytes(self.attributemask)
    }

    /// Retrieve a reference to the MRENCLAVE bytes
    pub fn mrenclave(&self) -> &[u8; MRENCLAVE_LEN] {
        &self.enclavehash
    }

    /// Generate the MRSIGNER value for this signature and return it as an array
    /// of bytes
    pub fn mrsigner(&self) -> [u8; MRSIGNER_LEN] {
        Sha256::digest(&self.modulus[..]).into()
    }

    /// Retrieve the product ID
    pub fn product_id(&self) -> u16 {
        u16::from_le_bytes(self.isvprodid)
    }

    /// Retrieve the product version
    pub fn version(&self) -> u16 {
        u16::from_le_bytes(self.isvsvn)
    }

    /// Retrieve a reference to the signature's "Q1" bytes
    pub fn q1(&self) -> &[u8; Q1_LEN] {
        &self.q1
    }

    /// Retrieve a reference to the signature's "Q2" bytes
    pub fn q2(&self) -> &[u8; Q2_LEN] {
        &self.q2
    }
}

impl TryFrom<&[u8]> for Signature {
    type Error = Error;

    fn try_from(src: &[u8]) -> Result {
        if src.len() < size_of::<Signature>() {
            return Err(Error::Length);
        }

        let (bytes, header) = take::<HEADER_LEN>(Some(validateHeaders))(src.as_bytes());
        let (bytes, vendor) = take::<VENDOR_LEN>(None)(bytes);
        let (bytes, date) = take::<DATE_LEN>(None)(bytes);
        let (bytes, header2) = take::<HEADER2_LEN>(None)(bytes);
        let (bytes, swdefined) = take::<SWDEFINED_LEN>(None)(bytes);
        let (bytes, _reserved1) = take::<RESERVED1_LEN>(None)(bytes);
        let (bytes, modulus) = take::<PUBKEY_LEN>(None)(bytes);
        let (bytes, exponent) = take::<EXPONENT_LEN>(None)(bytes);
        let (bytes, signature) = take::<SIGNATURE_LEN>(None)(bytes);
        let (bytes, miscselect) = take::<MISCSELECT_LEN>(None)(bytes);
        let (bytes, miscmask) = take::<MISCMASK_LEN>(None)(bytes);
        let (bytes, _reserved2) = take::<RESERVED2_LEN>(None)(bytes);
        let (bytes, attributes) = take::<ATTRIBUTES_LEN>(None)(bytes);
        let (bytes, attributemask) = take::<ATTRIBUTEMASK_LEN>(None)(bytes);
        let (bytes, enclavehash) = take::<MRENCLAVE_LEN>(None)(bytes);
        let (bytes, _reserved3) = take::<RESERVED3_LEN>(None)(bytes);
        let (bytes, isvprodid) = take::<ISVPRODID_LEN>(None)(bytes);
        let (bytes, isvsvn) = take::<ISVSVN_LEN>(None)(bytes);
        let (bytes, _reserved4) = take::<RESERVED4_LEN>(None)(bytes);
        let (bytes, q1) = take::<Q1_LEN>(None)(bytes);
        let (_bytes, q2) = take::<Q2_LEN>(None)(bytes);

        let vendor = u32::from_le_bytes(vendor).try_into()?;

        let signature = Signature {
            header,
            vendor,
            date,
            header2,
            swdefined,
            modulus,
            exponent,
            signature,
            miscselect,
            miscmask,
            attributes,
            attributemask,
            enclavehash,
            isvprodid,
            isvsvn,
            q1,
            q2,
        };

        if signature.header != HEADER1 {
            return Err(Error::BadHeader1);
        }

        // SGX enclaves built during Obama's second term are not viable.
        if u32::from_le_bytes(signature.date) < 0x2017_0120 {
            return Err(Error::BadDate);
        }

        if signature.header2 != HEADER2 {
            return Err(Error::BadHeader2);
        }

        // All reserved are all zero
        for reserved in [
            &_reserved1[..],
            &_reserved2[..],
            &_reserved3[..],
            &_reserved4[..],
        ]
            .iter()
        {
            if *reserved != &ZEROV[..reserved.len()] {
                return Err(Error::NonZeroReserved);
            }
        }

        Ok(signature)
    }
    fn validateHeaders(header: &[u8]) -> StdResult<&[u8], Error> {
        if header[..] != HEADER1[..] {
            return Err(Error::BadHeader1);
        }
        Ok(header)
    }
}


fn take<const SIZE: usize>(validation: Option<fn(&[u8]) -> StdResult<&[u8], Error>>) -> impl Fn(&[u8]) -> (&[u8], [u8; SIZE]) {
    move |input| {
        let parsing =
            nom::bytes::complete::take::<usize, &[u8], nom::error::Error<&[u8]>>(SIZE)(input)
                .expect("Size of stream should have been guaranteed to hold the bytes");
        let array = <[u8; SIZE]>::try_from(parsing.1)
            .expect("Size of stream should have been guaranteed to hold the bytes");
        match validation {
            Some(valid) => {
                valid(&array).expect("Validation failed");
                println!("Validation success")
            },
            None => println!("No validation function")
        };
        (parsing.0, array)
    }
}

// =======
// fn take(count: usize, validation: fn(&[u8]) -> Result) -> impl Fn(&[u8]) -> (&[u8], &[u8]) {
//     move |input| {
//         let parsing = nom::bytes::complete::take::<usize, &[u8], nom::error::Error<&[u8]>>(count)(input)
//             .expect("Size of stream should have been guaranteed to hold the bytes")
//         let array = <[u8; SIZE]>::tryz_from(parsing.1)
//             .expect("Size of stream should have been guaranteed to hold the bytes");
//         validation(array).expect("Validation failed");
//         (parsing.0, array)
//             >>>>>>> Stashed changes

impl Debug for Signature {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        f.debug_struct("Signature")
            .field("header", &format_args!("0x{}", HexFmt(&self.header)))
            .field("vendor", &self.vendor)
            .field("date", &format_args!("0x{:02x}", &self.date()))
            .field("header2", &format_args!("0x{}", HexFmt(&self.header2)))
            .field("swdefined", &format_args!("0x{}", HexFmt(&self.swdefined)))
            .field("MRSIGNER", &format_args!("0x{}", HexFmt(self.mrsigner())))
            // skipping exponent it's always 3
            .field("signature", &format_args!("0x{}", HexFmt(self.signature)))
            .field("miscselect", &self.misc_select())
            .field("miscmask", &self.misc_mask())
            .field("attributes", &self.attributes())
            .field("attributemask", &self.attribute_mask())
            .field("MRENCLAVE", &format_args!("0x{}", HexFmt(self.enclavehash)))
            .field("isvprodid", &self.product_id())
            .field("isvsvn", &self.version())
            .field("q1", &format_args!("0x{}", HexFmt(self.q1)))
            .field("q2", &format_args!("0x{}", HexFmt(self.q2)))
            .finish()
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    const VALID: &[u8] = include_bytes!("../tests/data/valid.css");
    const INVALID: &[u8] = include_bytes!("../tests/data/invalid.css");

    // values copied from the sgx_sign generated dump file at the time valid.css was
    // generated
    const VALID_PARSED: Signature = Signature {
        header: HEADER1,
        vendor: EnclaveVendor::Other,
        date: [0x28, 0x1, 0x20, 0x20],
        header2: HEADER2,
        swdefined: [0x0, 0x0, 0x0, 0x0],
        modulus: [
            ,
        ],
        exponent: [0x03, 0x00, 0x00, 0x00],
        signature: [
            ,
        ],
        miscselect: [0, 0, 0, 0],
        miscmask: [0xff, 0xff, 0xff, 0xff],
        attributes: [0x4, 0, 0, 0, 0, 0, 0, 0, 0x3, 0, 0, 0, 0, 0, 0, 0],
        attributemask: [
            0xfd, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x1b, 0xff, 0xff, 0xff, 0xff, 0xff,
            0xff, 0xff,
        ],
        enclavehash: [
            0x5b, 0x9f, 0xbf, 0xca, 0x6a, 0xc6, 0x2d, 0x82, 0x36, 0xec, 0x5e, 0x1c, 0x4e, 0xeb,
            0xd8, 0x47, 0xfb, 0x18, 0x07, 0x45, 0x0a, 0x78, 0xea, 0x24, 0x21, 0xbe, 0x5c, 0xb4,
            0xb4, 0x9a, 0x5e, 0xa9,
        ],
        isvprodid: [0x1, 0],
        isvsvn: [0x1, 0],
        q1: [
            ,
        ],
        q2: [
            ,
        ],
    };

    #[test]
    fn parse_valid() {
        let sig = Signature::try_from(VALID).expect("Could not parse valid CSS file");
        assert_eq!(VALID_PARSED, sig);

        assert_eq!(EnclaveVendor::Other, sig.vendor());
        assert_eq!(0x20200128, sig.date());
        assert_eq!(3, sig.pubkey_exponent());
        assert_eq!(0, sig.misc_select());
        assert_eq!(0xffff_ffff, sig.misc_mask());
        assert_eq!(
            0x0000_0000_0000_0003_0000_0000_0000_0004u128,
            sig.attributes()
        );
        assert_eq!(
            0xffff_ffff_ffff_ff1b_ffff_ffff_ffff_fffdu128,
            sig.attribute_mask()
        );
        assert_eq!(
            [
                0x7e, 0xe5, 0xe2, 0x9d, 0x74, 0x62, 0x3f, 0xdb, 0xc6, 0xfb, 0xf1, 0x45, 0x4b, 0xe6,
                0xf3, 0xbb, 0x0b, 0x86, 0xc1, 0x23, 0x66, 0xb7, 0xb4, 0x78, 0xad, 0x13, 0x35, 0x3e,
                0x44, 0xde, 0x84, 0x11,
            ],
            sig.mrsigner()
        );
        assert_eq!(1, sig.product_id());
        assert_eq!(1, sig.version());
    }

    #[test]
    fn invalid_css() {
        assert_eq!(Err(Error::Length), Signature::try_from(INVALID));
    }

    #[test]
    fn debug_output() {
        const VALID_DEBUG: &str = r#"
        Signature {
            header: 0x06000000e10000000000010000000000,
            vendor: Other,
            date: 0x20200128,
            header2: 0x01010000600000006000000001000000,
            swdefined: 0x00000000,
            MRSIGNER: 0x7ee5e29d74623fdbc6fbf1454be6f3bb0b86c12366b7b478ad13353e44de8411,
            signature: ,
            miscselect: 0,
            miscmask: 4294967295,
            attributes: 55340232221128654852,
            attributemask: 340282366920938459257516958625990443005,
            MRENCLAVE: 0x5b9fbfca6ac62d8236ec5e1c4eebd847fb1807450a78ea2421be5cb4b49a5ea9,
            isvprodid: 1,
            isvsvn: 1,
            q1: ,
            q2: ,
        }"#;
        let signature = Signature::try_from(VALID).expect("Could not parse valid CSS file");
        assert_eq!(format!("\n{signature:#?}"), textwrap::dedent(VALID_DEBUG));
    }
}
